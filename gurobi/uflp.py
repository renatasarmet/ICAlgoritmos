...
## For LP the last two are >= 0
## For MLP just xij >= 0

## And the Dual solves the following model:
# Dual LP:
#  maximize
#       SUM(􏰂vj)_{j \in D}
#  subject to
...
def dualLP(facility_list, client_list):
...
def facilityLP(facility_list, client_list):
...
def solve_it(input_type, input_data, formulation_type):
	...
	if facility_count * customer_count * facility_count <= memory_limit:
		...
		if(formulation_type == 'ILP'):
			...
			pair_new = facilityLP(facilities, customers)
			...
			pair_new = dualLP(facilities, customers)
