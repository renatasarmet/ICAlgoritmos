...
//2020_07 We have change this function to consider solution costs that are not
//only based on the resources acquired. For that reason decode_ap now must return
//the cost of the solution, considering the original costs, except resources chosen on
//first stage that cost 0 in the scenario.
double EF_local_search_solver::solve_all_scenarios(
		double (*decode_Ap)(EF::SP_instance &, int, std::vector <bool>&, std::vector <double> &,std::vector <bool>&),
	...
	for(int s = 0; s < problem.get_number_of_scenarios() ; s++){
		double scenario_total_cost = decode_Ap(problem, s, selected_resources, *perturbed_costs[s], solution);
