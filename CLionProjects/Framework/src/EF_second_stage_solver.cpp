...
double Brkga_Decoder::decode(std::vector< double >& chromosome) const {
	...
	total_cost = problem.decode_Ap(problem, scenario, selected_resources, allele_cost, resource_map);

	/*2020/07 We have decided that decode_Ap must return the total cost
	 * for (int i = 0; i < problem.get_number_of_resources(); i++){
