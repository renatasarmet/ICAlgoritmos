...
void Instance::readFrameworkInstance(int total_qty_facilities, int total_qty_clients,
                                     int qty_cli_scenario, int *exist_cli_scenario,
                                     double *allCostF, double **allCostA) {

    setQtyFacilities(total_qty_facilities);
    setQtyClients(qty_cli_scenario);

    allocateCosts();

//  Salvando os valores das instalações
    for(int f = 0; f < total_qty_facilities; f++){
        setCostFJ(f, allCostF[f]);
    }

//  Salvando os custos de conexoes
    int count = 0;
    for(int c = 0; c < total_qty_clients; c++){
        if(exist_cli_scenario[c]){
            for(int f = 0; f < total_qty_facilities; f++) {
                setCostAIJ(count,f,allCostA[c][f]);
            }
            count += 1;
        }
    }
