...
EXE = './Framework' # Executavel gerado pela compilação dos arquivos que realmente resolvem o problema
				...
				for i_start in inflation_start:
					for i_end in inflation_end:
						for chance in chance_cli_scenario:
							...
							time_limit = 600 # 10 minutes. -1 represents no time limit
							debug_type = 3
							solution_name = 'solutions/' + instance_name + '.sol'

							# Chamando o programa a ser testado
							os.system(EXE + " " + input_name + " " + str(time_limit) + " " + str(debug_type) + " > " + solution_name)
